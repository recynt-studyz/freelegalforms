...
const LOGO_KEY = 'flf-contractor-logo'
const TEMPLATES_KEY = 'flf-contractor-templates'
...
export default function ContractorAgreementGenerator() {
  ...
  const getFormData = () => ({
    clientName, clientAddress, clientContact, contractorName, contractorBusiness, contractorAddress,
    servicesDesc, deliverables, startDate, ongoing, endDate, paymentType, paymentAmount,
    paymentSchedule, lateFeeRate, ipOwnership, nonCompete, nonCompeteDuration, nonSolicitation,
    governingLaw, brandColor,
  })
  ...
  const loadTemplate = (t: Template) => {
    ...
    if (p.clientAddress !== undefined) setClientAddress(p.clientAddress)
    if (p.clientContact !== undefined) setClientContact(p.clientContact)
    if (p.contractorName !== undefined) setContractorName(p.contractorName)
    if (p.contractorBusiness !== undefined) setContractorBusiness(p.contractorBusiness)
    if (p.contractorAddress !== undefined) setContractorAddress(p.contractorAddress)
    if (p.servicesDesc !== undefined) setServicesDesc(p.servicesDesc)
    if (p.deliverables !== undefined) setDeliverables(p.deliverables)
    if (p.startDate) setStartDate(p.startDate)
    if (p.ongoing !== undefined) setOngoing(p.ongoing)
    if (p.endDate !== undefined) setEndDate(p.endDate)
    if (p.paymentType) setPaymentType(p.paymentType)
    if (p.paymentAmount !== undefined) setPaymentAmount(p.paymentAmount)
    if (p.paymentSchedule) setPaymentSchedule(p.paymentSchedule)
    if (p.lateFeeRate !== undefined) setLateFeeRate(p.lateFeeRate)
    if (p.ipOwnership) setIpOwnership(p.ipOwnership)
    if (p.nonCompete !== undefined) setNonCompete(p.nonCompete)
    if (p.nonCompeteDuration !== undefined) setNonCompeteDuration(p.nonCompeteDuration)
    if (p.nonSolicitation !== undefined) setNonSolicitation(p.nonSolicitation)
    if (p.governingLaw) setGoverningLaw(p.governingLaw)
